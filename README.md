...
| 이름   | 기능 및 역할 |
| ----|--------------------------------------------------------------------------- |
| 서은진 |  팀장, BackEnd </br> SMTP를 활용한 회원가입/비밀번호 초기화, Spring Security, JWT를 활용한 로그인, 회원 스트릭 생성, 주간 TEST 시작 및 결과 MongoDB저장 및 조회, 데이터베이스 설계, 업적달성 및 조회, 회원정보 조회/수정, 테스트 통계 제공, 게임 통계 제공|
